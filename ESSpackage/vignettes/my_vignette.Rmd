...
\end{align*}
4. Propose a new status $\mathbf{f^{'}}\leftarrow\mathbf{f}\text{cos}\theta+\pmb{\nu}\text{sin}\theta$
5. If $\text{log}L(\mathbf{f^{'}})>\text{log}y$ then:
6.    $\qquad$Accept: return $\mathbf{f^{'}}$
7. Else:
    $\qquad$Shrink the bracket:
8. $\qquad$if $\theta<0$ then: $\theta_{min}\leftarrow\theta$, else $\theta_{max}\leftarrow\theta$
9. $\qquad\theta\sim\text{Uniform}[\theta_{min}, \theta_{max}]$
10. $\qquad$Go to 4.
...
This will lead to the reversibility of the algorithm by substituting $\ref{angles}$ and $\ref{gaussians}$ in $\ref{newtarget}$.
